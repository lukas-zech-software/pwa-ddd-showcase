...
@observer
class _CoronaTakeAwayDetailsCard extends React.Component<Props, State> {
    ...
    return (
      <>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <CompanyFormCheckbox propertyName="offersTakeAway"
                                 label={locale.forms.corona.takeAway.checkbox}
              /*!! because value maybe undefined --> uncontrolled */
                                 value={!!company.corona.offersTakeAway}
                                 onCheckboxChange={(checked) => company.corona.offersTakeAway = checked}
                                 controlLabelRootClassName={classes.controlLabelRootClassName}
                                 errorMessage={undefined}
            />
            <FormTooltip title={locale.forms.corona.takeAway.checkboxTooltip}
                         inline={width !== 'xs'}
            />
          </Grid>
          ...
          <Grid item xs={12}>
            <Typography variant="caption" component={'p' as any}>
              {locale.forms.corona.takeAway.description}
            </Typography>
          </Grid>
            ...
            <Typography onClick={() => {
              this.setDialogContent(
                <Paragraphs className={classes.para}
                            variant="body2">{locale.forms.corona.takeAway.descriptionHintLink1Text}</Paragraphs>,
                locale.forms.corona.takeAway.descriptionHintLink1,
              );
          ...
          <Grid item xs={12}>
            ...
            <CompanyFormField
              onClick={(e) => {
                if (!company.corona.offersTakeAway) {
                  company.corona.offersTakeAway = true;
                }
              }}
              disabled={!company.corona.offersTakeAway}
              errorMessage={getValidationError(errors, 'takeAwayDescription')}
              value={company.corona.takeAwayDescription || ''}
              propertyName="takeAwayDescription"
              onValueChange={(value) => company.corona.takeAwayDescription = value}
              label={locale.forms.corona.takeAway.textLabel}
              multiline
              rows="10"/>
...
export const CoronaTakeAwayDetailsCard = withWidth()(withStyles(styles)(_CoronaTakeAwayDetailsCard));
